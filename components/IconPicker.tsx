        ...
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in" onClick={onClose}>
            <div className="bg-[#1e1e2e] w-full max-w-md rounded-2xl p-6 border border-white/10 shadow-2xl flex flex-col max-h-[80vh]" onClick={e => e.stopPropagation()}>
                <div className="flex justify-between items-center mb-4">
                    <h3 className="text-lg font-bold text-white">Válassz Szimbólumot</h3>
                    <div className="flex bg-black/40 p-1 rounded-lg border border-white/10">
                        <button onClick={() => setTab('icons')} className={`px-3 py-1 rounded-md text-[10px] font-bold uppercase transition-all ${tab === 'icons' ? 'bg-gold-500 text-black' : 'text-white/40 hover:text-white'}`}>Ikonok</button>
                        <button onClick={() => setTab('emojis')} className={`px-3 py-1 rounded-md text-[10px] font-bold uppercase transition-all ${tab === 'emojis' ? 'bg-gold-500 text-black' : 'text-white/40 hover:text-white'}`}>Emojik</button>
                    </div>
                </div>
                ...
                {tab === 'icons' && (
                    <>
                        <input
                            autoFocus
                            placeholder="Keresés az ikonok között..."
                            value={search}
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar">
                    {tab === 'icons' ? renderIcons() : (
                        <div className="grid grid-cols-6 gap-3 p-2">
                            {EMOJIS.map(emoji => (
                                <button
                                    key={emoji}
                                    onClick={() => { onSelect(emoji); onClose(); }}
                                    className="aspect-square bg-white/5 hover:bg-gold-500/20 hover:border-gold-500 border border-white/5 rounded-xl flex items-center justify-center text-2xl transition-all"
                                >
                                    {emoji}
                                </button>
                            ))}
                ...
                <div className="mt-4 pt-4 border-t border-white/5 text-right">
                    <button onClick={onClose} className="text-white/40 hover:text-white text-xs font-bold uppercase tracking-widest transition-colors">Mégse</button>
                </div>
            </div>
        </div>
    );
